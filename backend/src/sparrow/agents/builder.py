...
from sparrow.agents.base import FIDELITY_LINE, Agent, Result, context_block
...
class BuildOutput:
    def __init__(self, code: str, extension_request: str | None, usage: Result) -> None:
        self.code = code
...
class Builder(Agent):
    ...
    def build(
        ...
        *,
        ...
        ground = (
            "the page ground (`bg-background`)"
            if section.ground is Ground.PAGE
            else "the muted ground (`bg-muted`)"
        )
        ...
        user = "\n\n".join([
            ...
            f"ground: this section sits on {ground} — decided at page level, not by you\n"
            f"</section>",
            f"<stack>\n{stack.strip()}\n"
